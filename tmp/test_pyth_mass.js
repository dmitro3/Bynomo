...
const PRICE_FEED_IDS = {
  ...
  APT: '0x03ae489113730ca88c0944020f3214da8718bd4d1c68c1fdc6f2ee672f0bc0b0',
  // Metals
...
async function testAllIds() {
  ...
  // Test chunking (14 IDs like in the log)
  const chunk = ids.slice(0, 14);
  const query = chunk.map(id => `ids[]=${id}`).join('&');
  const chunkUrl = `${baseUrl}?${query}`;
  try {
    const resp = await fetch(chunkUrl);
    console.log(`CHUNK TEST: ${chunkUrl} -> ${resp.status} ${resp.statusText}`);
  } catch (err) {
